...
export function getUseLadyOfTheLakePrompt(context: LadyContext): string[] {
    const historyText = context.ladyHistory.map(h => `${context.playerNames[h.holderId] || h.holderId} inspected ${context.playerNames[h.targetId] || h.targetId}, claimed: ${h.claim}`).join('\n');
    ...
    return [
        ...
        `Respond in strict JSON: {"targetId": "pX"}`
    ];
}
